...
#define LCD_CMD_GC9A01_MADCTL    0x36
#define LCD_CMD_GC9A01_COLMOD    0x3A
#define LCD_CMD_GC9A01_CASET     0x2A
#define LCD_CMD_GC9A01_RASET     0x2B
#define LCD_CMD_GC9A01_RAMWR     0x2C
#define LCD_CMD_GC9A01_INVON     0x21
#define LCD_CMD_GC9A01_INVOFF    0x20
#define LCD_CMD_GC9A01_DISPON    0x29
#define LCD_CMD_GC9A01_DISPOFF   0x28
...
esp_err_t esp_lcd_new_panel_gc9a01(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    case 18: // RGB666
        ...
        gc9a01->fb_bits_per_pixel = 18;
        break;
    case 24: // RGB888
        gc9a01->colmod_cal = 0x77;
...
static esp_err_t panel_gc9a01_reset(esp_lcd_panel_t *panel)
{
    ...
    if (gc9a01->reset_gpio_num >= 0) {
        ...
        esp_lcd_panel_io_tx_param(io, LCD_CMD_SWRST, NULL, 0);
...
static esp_err_t panel_gc9a01_init(esp_lcd_panel_t *panel)
{
    ...
    esp_lcd_panel_io_tx_param(io, LCD_CMD_GC9A01_MADCTL, (uint8_t[]){0x18}, 1);
    esp_lcd_panel_io_tx_param(io, LCD_CMD_GC9A01_COLMOD, (uint8_t[]){gc9a01->colmod_cal}, 1);
    ...
    esp_lcd_panel_io_tx_param(io, LCD_CMD_GC9A01_DISPON, NULL, 0);
...
static esp_err_t panel_gc9a01_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    esp_lcd_panel_io_tx_param(io, LCD_CMD_GC9A01_CASET, (uint8_t[]) {
        (x_start >> 8) & 0xFF,
        x_start & 0xFF,
        ((x_end - 1) >> 8) & 0xFF,
        (x_end - 1) & 0xFF,
    }, 4);
    esp_lcd_panel_io_tx_param(io, LCD_CMD_GC9A01_RASET, (uint8_t[]) {
        (y_start >> 8) & 0xFF,
        y_start & 0xFF,
        ((y_end - 1) >> 8) & 0xFF,
        (y_end - 1) & 0xFF,
    ...
    esp_lcd_panel_io_tx_color(io, LCD_CMD_GC9A01_RAMWR, color_data, len);
...
static esp_err_t panel_gc9a01_invert_color(esp_lcd_panel_t *panel, bool invert_color_data)
{
    ...
    if (invert_color_data) {
        command = LCD_CMD_GC9A01_INVON;
    } else {
        command = LCD_CMD_GC9A01_INVOFF;
    }
...
static esp_err_t panel_gc9a01_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    esp_lcd_panel_io_tx_param(io, LCD_CMD_GC9A01_MADCTL, (uint8_t[]) {gc9a01->madctl_val}, 1);
    return ESP_OK;
}
...
static esp_err_t panel_gc9a01_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    ...
    int command = 0;
    if (on_off) {
        command = LCD_CMD_GC9A01_DISPON;
    } else {
        command = LCD_CMD_GC9A01_DISPOFF;
    }
    esp_lcd_panel_io_tx_param(io, command, NULL, 0);
    return ESP_OK;
}